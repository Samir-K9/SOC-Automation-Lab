...
![Workflow Overview](https://github.com/Samir-K9/SOC-Automation-Lab/blob/main/Screenshots/Screenshot%202024-12-23%20132100.png?raw=true)
...
- Finally run Powershell as Administrator and change to the same directory as Sysmon and install it using the command:
...
![Sysmon service](https://github.com/Samir-K9/SOC-Automation-Lab/blob/main/Screenshots/VirtualBox_Demo_01_01_2025_19_19_19.png?raw=true)
...
```
  ...
  - Change `listen_address` to Public IP of TheHive
  - Change `rpc_address` to Public IP of TheHive
  - Change `seeds` to Public IP of TheHive (e.g. `178.128.228.152:7000`)

- Stop Cassandra Service, remove old files, then restart:
...
```
  ...
  - Remove the comment and change `network.host` to Public IP of TheHive
...
- Make sure thehive user and group have access to the required file path:
...
```
  ...
  - Change database and index configuration hostnames to Public IP of TheHive
  - Change `cluster-name` to same as Cassandra (e.g. `Test Cluster`)
  - Change `application.baseURL` to Public IP of TheHive (e.g. `http://178.128.228.152:9000`)
...
- Navigate to the public IP of TheHive with port 9000 and login with the default credentials: `admin@thehive.local` / `secret`
...
- Configure Wazuh to log this event by making some changes:
  - Connect to Wazuh Server via SSH and make changes to `ossec.conf`:
...
```
  ...
  - Change `enabled: false` to `true`, save the file and restart Filebeat:
...
- Head over to TheHive and login with the default credentials: `admin@thehive.local` / `secret`
...
- Add users to this newly created organization. Add one analyst account and one for SOAR.
...
- Set password for the analyst account and create an API key for SOAR and copy it.
...
- Rerun the workflow and an alert should popup on TheHive if everything is working properly.
